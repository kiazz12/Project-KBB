...
export class FormService extends BaseService {
  ...
  exportPdf(id: number): Promise<Blob> {
    return this.getBlob(`/${id}/export/pdf`);
  }
  ...
  submitPublicForm(slug: string, data: Record<string, unknown>): Promise<{ uuid: string }> {
    return this.post<{ uuid: string }>(`/public/${slug}`, data);
  }
