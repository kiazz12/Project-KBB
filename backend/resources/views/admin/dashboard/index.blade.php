    ...
    <div class="group bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 mt-1.5">{{ $totalPublished }}</p>
            </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-5">Forms Terbaru</h2>
        @if ($recentForms->isEmpty())
            <p class="text-sm text-gray-400 text-center py-10">Belum ada form.</p>
        @else
            <div class="space-y-1.5">
                @foreach ($recentForms as $form)
                    <a href="{{ route('admin.forms.show', $form) }}" class="flex items-center justify-between p-3.5 rounded-xl hover:bg-gray-50 transition-all group">
                        <div class="flex items-center gap-3.5 min-w-0">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-gray-400 mt-0.5">
                                    <span class="font-medium text-gray-600">{{ $form->submissions_count }}</span> pengiriman
                                    <span class="text-gray-300">&middot;</span>
                                    {{ $form->user?->name ?? 'Unknown' }}
                                </p>
                            </div>
                        </div>
                        <span class="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full font-semibold flex-shrink-0 {{ $form->status->value === 'published' ? 'bg-emerald-50 text-emerald-700' : ($form->status->value === 'closed' ? 'bg-red-50 text-red-700' : 'bg-gray-100 text-gray-500') }}">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-5">Users by Role</h2>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-medium text-gray-700 capitalize">{{ str_replace('_', ' ', $roleVal) }}</span>
                            </div>
