                    ...
                    <div class="flex items-center gap-2">
                        @foreach ($steps as $i => $step)
                            <div class="flex items-center gap-2 flex-1">
                                <div class="flex items-center gap-2">
                                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold {{ $i + 1 === $currentStep ? 'bg-kbb-700 text-white' : ($i + 1 < $currentStep ? 'bg-emerald-500 text-white' : 'bg-gray-200 text-gray-500') }}">
                                        @if ($i + 1 < $currentStep)
                                    ...
                                    <span class="text-xs font-medium {{ $i + 1 === $currentStep ? 'text-kbb-700' : 'text-gray-400' }} hidden sm:inline">{{ $step['title'] }}</span>
                                </div>
                                @if ($i < $totalSteps - 1)
                                    <div class="flex-1 h-0.5 {{ $i + 1 < $currentStep ? 'bg-emerald-500' : 'bg-gray-200' }}"></div>
                                @endif
                            </div>
                        @endforeach
                    </div>
                        ...
                        <div>
                            @if ($currentStep < $totalSteps)
                                <button type="button" wire:click="nextStep" wire:loading.attr="disabled"
                                    class="bg-kbb-700 hover:bg-kbb-800 disabled:opacity-50 text-white font-medium px-6 py-2.5 rounded-lg transition text-sm">
