<div class="w-full max-w-2xl mx-auto">
    ...
    @if ($form && !$submitted)
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
            {{-- Header --}}
            <div class="relative bg-gradient-to-br from-[#003778] via-[#004a9c] to-[#003778] px-8 py-8">
            ...
            <div class="px-8 py-5">
                <form wire:submit="submitForm">
                            ...
                            @foreach ($step['fields'] as $field)
                                ...
                                <div class="group">
                                    @if ($field->type->value !== 'computed')
                                        <label class="block text-sm font-semibold text-gray-700 mb-1.5 font-sans">
                                            {{ $field->label }}
                                            @if ($field->required) <span class="text-red-400">*</span> @endif
                                        </label>
                                    @endif
                                    @if ($field->help_text)
                                        <p class="text-xs text-gray-400 mb-1.5 -mt-0.5">{{ $field->help_text }}</p>
                                    @endif
                                    ...
                                    @if (in_array($field->type->value, ['text', 'email', 'number']))
                                        <input type="{{ $field->type->value === 'email' ? 'email' : ($field->type->value === 'number' ? 'number' : 'text') }}"
                                            ...
                                            class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder-gray-300
                                    ...
                                    @if ($field->type->value === 'select')
                                        <select wire:model="responses.{{ $field->id }}" @required($field->required)
                                            class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-800
                                        ...
                                        <div class="flex flex-wrap gap-2 mt-1">
                                            @foreach ($field->options ?? [] as $opt)
                                                <label class="relative cursor-pointer">
                                                    ...
                                                    <div class="px-4 py-2 rounded-xl border-2 border-gray-200 text-sm font-medium text-gray-500
                                    ...
                                    @if ($field->type->value === 'file')
                                        <input type="file" wire:model="responses.{{ $field->id }}" @required($field->required) accept=".jpg,.jpeg,.png,.pdf,.doc,.docx"
                                            class="w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-[#003778]/10 file:text-[#003778] hover:file:bg-[#003778]/20 transition">
                                    ...
                                    @if ($field->type->value === 'heading')
                                        <h3 class="text-lg font-bold text-gray-900 font-display">{{ $field->label }}</h3>
                                    @endif

                                    {{-- Paragraph --}}
                                    @if ($field->type->value === 'paragraph')
                                        <div class="bg-blue-50/50 border border-blue-100 rounded-xl px-4 py-3">
                                            <p class="text-sm text-gray-600 leading-relaxed">{{ $field->label }}</p>
                                        </div>
                                    @endif
                                    ...
                                    @if ($field->type->value === 'computed')
                                        <div class="bg-gradient-to-r from-[#003778]/5 to-transparent border border-[#003778]/10 rounded-xl px-4 py-3">
                                            <span class="text-xs text-gray-400 block mb-0.5">{{ $field->label }}</span>
                                            <span class="text-base font-bold text-[#003778] font-display">
                    ...
                    {{-- Error --}}
                    @if ($error)
                        <div class="flex items-center gap-2 px-4 py-3 bg-red-50 border border-red-200 text-red-600 rounded-xl text-sm mb-4">
                        ...
                        <div>
                            @if ($currentStep > 1)
                                <button type="button" wire:click="prevStep"
                                    class="flex items-center gap-1.5 bg-gray-50 hover:bg-gray-100 text-gray-500 font-semibold px-5 py-2.5 rounded-xl transition-all duration-200 text-sm border border-gray-200">
                        ...
                        <div>
                            @if ($currentStep < $totalSteps)
                                <button type="button" wire:click="nextStep" wire:loading.attr="disabled"
                                    class="flex items-center gap-1.5 bg-[#003778] hover:bg-[#002a5c] disabled:opacity-50 text-white font-semibold px-6 py-2.5 rounded-xl transition-all duration-200 text-sm shadow-md shadow-[#003778]/20">
                            ...
                            @else
                                <button type="submit" wire:loading.attr="disabled"
                                    class="flex items-center gap-2 bg-gradient-to-r from-[#003778] to-[#004a9c] hover:from-[#002a5c] hover:to-[#003778] disabled:opacity-50 text-white font-bold px-8 py-2.5 rounded-xl transition-all duration-200 text-sm shadow-lg shadow-[#003778]/25">
    ...
    @if ($submitted)
        <div class="bg-white rounded-2xl shadow-xl p-10 text-center border border-gray-100">
            ...
            <h2 class="font-display text-2xl font-extrabold text-gray-900 mb-2 tracking-tight">Terima Kasih!</h2>
            <p class="text-sm text-gray-500 max-w-sm mx-auto leading-relaxed">{{ $form?->confirmation_message ?? 'Jawaban Anda telah berhasil dikirim dan dicatat.' }}</p>
