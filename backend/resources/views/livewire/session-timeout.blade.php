<div wire:poll.30s="checkSession" id="session-timeout-component">
    ...
    @if ($showWarning && !$expired)
        <div id="session-timeout-modal" class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm" style="animation: stFadeIn 0.2s ease-out">
            <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-4 overflow-hidden" style="animation: stIn 0.3s ease-out">
                {{-- Header --}}
                <div class="px-6 pt-6 pb-4 text-center">
                    ...
                    <h3 class="text-lg font-bold text-gray-900 mb-1">Session Akan Habis</h3>
                    <p class="text-sm text-gray-500">Sesi Anda akan berakhir dalam <span id="session-countdown" class="font-semibold text-amber-600">{{ floor($remainingSeconds / 60) }}:{{ str_pad($remainingSeconds % 60, 2, '0', STR_PAD_LEFT) }}</span></p>
                </div>
    ...
    @if ($expired)
        <div id="session-expired-modal" class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-4 p-6 text-center" style="animation: stIn 0.3s ease-out">
                ...
                <h3 class="text-lg font-bold text-gray-900 mb-2">Session Berakhir</h3>
                <p class="text-sm text-gray-600 mb-6">Sesi Anda telah berakhir karena tidak ada aktivitas. Silakan login kembali untuk melanjutkan.</p>
